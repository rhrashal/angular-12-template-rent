...
let VERSIONED_FILES = ['bower.json', 'package.json'];
...
export function version() {
  return (
    gulp
      .src(VERSIONED_FILES, { base: process.cwd() })
      //.pipe(replace(CURRENT_VERSION, NEXT_VERSION))
      .pipe(
        replace(
          /("|')version\1\s*:\s*("|')([\d.]+)\2/,
          `$1version$1:$2${NEXT_VERSION}$2`
        )
      )
      .pipe(gulp.dest('.'))
  );
...
export function pull(done) {
  ...
  if (gitStatusResult.toString().length > 0) {
    return fail(
      'You have uncommitted changes, please stash or commit them before running prepare'
    );
  }
