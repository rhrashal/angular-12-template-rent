...
export function bundler(
  ...
  input = config.scripts.input,
  files = config.scripts.files,
  message = 'Bundler task complete'
) {
  return function () {
  ...
  return function () {
    ...
    return (
      gulp
        ...
        .pipe(header(config.banner))
        .pipe(
          gulpif(createSourcemap, sourcemaps.write(config.deploy ? './' : null))
        )
        .pipe(gulp.dest(dest))
        .pipe(
          size({
...
export function version(
  src = config.scripts.src,
  file = config.scripts.version
) {
  return function () {
    return gulp
      .src(path.join(src, file), { base: './' })
      .pipe(
        replace(
          /("{0,1}|'{0,1})version\1\s*:\s*("|')([\d.]+)\2/,
          `$1version$1:$2${config.version}$2`
        )
      )
