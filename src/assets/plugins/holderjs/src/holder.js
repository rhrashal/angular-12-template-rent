...
(function (register, global, undefined) {
  ...
  function prepareDOMElement(mode, el, flags, _renderSettings) {
    ...
    if (mode == 'image' || mode == 'fluid') {
      setAttr(el, {
        alt: theme.text
          ? (theme.text.length > 16 ? theme.text.substring(0, 16) + '…' : theme.text) + ' [' + dimensionsCaption + ']'
          : dimensionsCaption,
      });
    }
  ...
  var sgSVGRenderer = (function () {
    ...
    return function (sceneGraph, renderSettings) {
      ...
      var svgString =
        'data:image/svg+xml;base64,' + btoa(unescape(encodeURIComponent(serializeSVG(svg, renderSettings))));
      return svgString;
    };
  })();
