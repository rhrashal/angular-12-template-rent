...
(function (h) {
    ...
    if (d || (!r && !f && !e && j == ba(a) && j == n.length))
      ...
      for (m = 0; m < i.length; m++)
        (p = c[i[m]]),
          ...
          p.sWidthOrig &&
            f &&
            h(n[m]).append(h('<div/>').css({ width: p.sWidthOrig, margin: 0, padding: 0, border: 0, height: 1 }));
      if (a.aoData.length)
        for (m = 0; m < i.length; m++)
      ...
      b = function (b) {
        if (b && b.time) {
          ...
          if (
            -1 === h.inArray(!1, i) &&
            ((i = a.iStateDuration),
            !(0 < i && b.time < +new Date() - 1e3 * i) && !(b.columns && f.length !== b.columns.length))
          ) {
  ...
  function ia(a, b) {
    ...
    b <= c
      ...
      : (a >= b - 1 - d ? (c = X(b - (c - 2), b)) : ((c = X(a - d + 2, a + d - 1)), c.push('ellipsis'), c.push(b - 1)),
