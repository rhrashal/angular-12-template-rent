  ...
  Echarts.prototype = {
    ...
    _eventPackage: function (target) {
      if (target) {
        ...
        dataIndex =
          seriesIndex != -1 && this.component.dataZoom
            ? this.component.dataZoom.getRealDataIndex(seriesIndex, dataIndex)
            : dataIndex;
        return {
    ...
    _noDataCheck: function (magicOption) {
      ...
      var loadOption = (this._option && this._option.noDataLoadingOption) ||
        ...
        ecConfig.noDataLoadingOption || {
          ...
          effect:
            (this._option && this._option.noDataEffect) || this._themeConfig.noDataEffect || ecConfig.noDataEffect,
        };
      this.clear();
    ...
    _mergeGlobalConifg: function (magicOption) {
      ...
      while (len--) {
        ...
        if (magicOption[mergeItem] == null) {
          magicOption[mergeItem] =
            this._themeConfig[mergeItem] != null ? this._themeConfig[mergeItem] : ecConfig[mergeItem];
        }
      }
      ...
      for (var i = 0, l = params.length; i < l; i++) {
        ...
        if (seriesItem) {
          ...
          if (additionData != null) {
            ...
            var legendData;
            if (
              seriesItem.type === ecConfig.CHART_TYPE_PIE &&
    ...
    showLoading: function (loadingOption) {
      ...
      var finalTextStyle = zrUtil.merge(
        zrUtil.merge(zrUtil.clone(textStyle), this._themeConfig.textStyle),
        ecConfig.textStyle
      );
      textStyle.textFont =
        finalTextStyle.fontStyle +
        ' ' +
        finalTextStyle.fontWeight +
        ' ' +
        finalTextStyle.fontSize +
        'px ' +
...
define('zrender/tool/event', ['require', '../mixin/Eventful'], function (require) {
  ...
  function getX(e) {
    return (
      (typeof e.zrenderX != 'undefined' && e.zrenderX) ||
  ...
  function getY(e) {
    return (
      (typeof e.zrenderY != 'undefined' && e.zrenderY) ||
  ...
  function getDelta(e) {
    return (
      (typeof e.zrenderDelta != 'undefined' && e.zrenderDelta) ||
...
define('zrender/zrender', [
  ...
  ZRender.prototype.animate = function (el, path, loop) {
    ...
    if (el) {
      ...
      var animator = this.animation
        .animate(target, { loop: loop })
        .during(function () {
          self.modShape(el);
        })
        .done(function () {
      ...
      if (this._markStart) {
        ...
        if (this._iconShapeMap['mark']) {
          this.zr.modShape(this._iconShapeMap['mark'].id, {
            style: { strokeColor: this._iconShapeMap['mark'].highlightStyle.strokeColor },
          });
        }
      ...
      if (this._zoomStart) {
        ...
        if (this._iconShapeMap['dataZoom']) {
          this.zr.modShape(this._iconShapeMap['dataZoom'].id, {
            style: { strokeColor: this._iconShapeMap['dataZoom'].highlightStyle.strokeColor },
          });
        }
    ...
    __onSaveAsImage: function () {
      ...
      if (!this.myChart.isConnected()) {
        image = this.zr.toDataURL(
          'image/' + imgType,
          this.option.backgroundColor && this.option.backgroundColor.replace(' ', '') === 'rgba(0,0,0,0)'
            ? '#fff'
      ...
      if (this.query(newOption, 'toolbox.show') && this.query(newOption, 'toolbox.feature.magicType.show')) {
        ...
        while (len--) {
          ...
          if (this._magicMap[oriType]) {
            axis =
              newOption.xAxis instanceof Array
            ...
            axis =
              newOption.yAxis instanceof Array
...
define('echarts/component/title', [
  ...
  Title.prototype = {
    ...
    _getItemGroupLocation: function () {
      ...
      var totalHeight =
        zrArea.getTextHeight(text, font) +
  ...
  Tooltip.prototype = {
    ...
    __tryShow: function () {
      ...
      if (!this._curTarget) {
        ...
        if (serie == null || data == null || !needShow) {
          ...
          trigger === 'axis'
            ? this._showAxisTrigger(serie.xAxisIndex, serie.yAxisIndex, ecData.get(this._curTarget, 'dataIndex'))
            : this._showItemTrigger();
        }
      }
    },
        ...
        if (this.deepQuery([series[i], this.option], 'tooltip.trigger') === 'axis') {
          ...
          if (
            ...
            this._showAxisTrigger(
              xAxisIndex,
              yAxisIndex,
              this._getNearestDataIndex('x', this.component.xAxis.getAxis(xAxisIndex))
            );
            return;
          } else if (
            ...
            this._showAxisTrigger(
              xAxisIndex,
              yAxisIndex,
              this._getNearestDataIndex('y', this.component.yAxis.getAxis(yAxisIndex))
            );
            return;
          } else {
    ...
    _showAxisTrigger: function (xAxisIndex, yAxisIndex, dataIndex) {
      !this._event.connectTrigger &&
        this.messageCenter.dispatch(ecConfig.EVENT.TOOLTIP_IN_GRID, this._event, null, this.myChart);
      ...
      var axisLayout =
        xAxisIndex != -1 && this.component.xAxis.getAxis(xAxisIndex).type === ecConfig.COMPONENT_TYPE_AXIS_CATEGORY
          ? 'xAxis'
          : yAxisIndex != -1 && this.component.yAxis.getAxis(yAxisIndex).type === ecConfig.COMPONENT_TYPE_AXIS_CATEGORY
          ? 'yAxis'
          : false;
      var x;
      var y;
      ...
      if (this._lastItemTriggerId !== this._curTarget.id) {
        ...
        if (typeof formatter === 'function') {
          ...
          formatter = formatter
            ...
            .replace('{c0}', value instanceof Array ? value : this.numAddCommas(value));
    ...
    _styleAxisPointer: function (seriesArray, xStart, yStart, xEnd, yEnd, gap, x, y) {
      if (seriesArray.length > 0) {
        ...
        for (var i = 0, l = seriesArray.length; i < l; i++) {
          ...
          if (curType) {
            style[curType].color =
              this.query(queryTarget, 'tooltip.axisPointer.' + curType + 'Style.color') || style[curType].color;
            style[curType].width =
              this.query(queryTarget, 'tooltip.axisPointer.' + curType + 'Style.width') || style[curType].width;
            style[curType].type =
              this.query(queryTarget, 'tooltip.axisPointer.' + curType + 'Style.type') || style[curType].type;
          }
        }
        if (pointType === 'line') {
    ...
    __onmousemove: function (param) {
      ...
      if (!target) {
        ...
        if (
          ...
          !this._event.connectTrigger &&
            this.messageCenter.dispatch(ecConfig.EVENT.TOOLTIP_OUT_GRID, this._event, null, this.myChart);
          this._hidingTicket = setTimeout(this._hide, this._hideDelay);
        }
      } else {
    ...
    _buildItem: function () {
      ...
      for (var i = 0; i < dataLength; i++) {
        ...
        if (this.legendOption.orient === 'horizontal') {
          ...
          if (
            zrHeight - lastY < 200 &&
            itemHeight + (i === dataLength - 1 || data[i + 1] === '' ? 0 : itemGap) >= zrHeight - lastY
          ) {
            this.legendOption.x === 'right'
              ? (lastX -= this._itemGroupLocation.maxWidth + itemGap)
              : (lastX += this._itemGroupLocation.maxWidth + itemGap);
      ...
      if (
        this.legendOption.orient === 'horizontal' &&
          ...
          lineOffsetArray.push(
            (this._itemGroupLocation.width -
              (this.shapeList[i - 1].style.x +
                zrArea.getTextWidth(this.shapeList[i - 1].style.text, this.shapeList[i - 1].style.textFont) -
                lastX)) /
              2
          );
        } else if (i === l - 1) {
          lineOffsetArray.push(
            (this._itemGroupLocation.width -
              (this.shapeList[i].style.x +
                zrArea.getTextWidth(this.shapeList[i].style.text, this.shapeList[i].style.textFont) -
                lastX)) /
              2
          );
        }
      ...
      if (this.legendOption.orient === 'horizontal') {
        ...
        for (var i = 0; i < dataLength; i++) {
          ...
          var tempTextWidth = zrArea.getTextWidth(
            ...
            data[i].textStyle ? this.getFont(zrUtil.merge(data[i].textStyle || {}, textStyle)) : font
          );
    ...
    _getItemShapeByType: function (x, y, width, height, color, itemType, defaultColor) {
      ...
      switch (itemType) {
        ...
        case 'k':
          ...
          itemShape.highlightStyle.lineWidth = 3;
          itemShape.highlightStyle.color = itemShape.style.color =
            this.deepQuery([this.ecTheme, ecConfig], 'k.itemStyle.normal.color') || '#fff';
          itemShape.style.strokeColor =
            color != '#ccc'
              ? this.deepQuery([this.ecTheme, ecConfig], 'k.itemStyle.normal.lineStyle.color') || '#ff3200'
              : color;
          break;
        case 'image':
      ...
      if (newOption) {
        ...
        if (this.legendOption.selected) {
          for (var k in this.legendOption.selected) {
            this._selectedMap[k] =
              typeof this._selectedMap[k] != 'undefined' ? this._selectedMap[k] : this.legendOption.selected[k];
          }
        }
    ...
    self._setCurrentOption = function () {
      ...
      self.messageCenter.dispatch(
        ...
        null,
        {
          ...
          data:
            timelineOption.data[self.currentIndex].name != null
  ...
  Timeline.prototype = {
    ...
    _getChainPoint: function () {
      ...
      if (len > 1) {
        ...
        if (timelineOption.type === 'number') {
          ...
          for (var i = 1; i < len; i++) {
            xList[i] =
              x + boundaryGap + (width * (new Date(_getName(i).replace(/-/g, '/')) - xList[0])) / xList[len - 1];
          }
    ...
    _syncHandleShape: function () {
      ...
      this._handleShape.style.textColor =
        cpStyle.label.textStyle.color === 'auto' ? this._handleShape.style.color : cpStyle.label.textStyle.color;
      this._handleShape.highlightStyle.strokeColor = this._handleShape.style.strokeColor =
        cpStyle.borderColor === 'auto' ? (curPoint.borderColor ? curPoint.borderColor : '#fff') : cpStyle.borderColor;
      this._handleShape.style.lineWidth =
        cpStyle.borderWidth === 'auto' ? (curPoint.borderWidth ? curPoint.borderWidth : 0) : cpStyle.borderWidth - 0;
      ...
      contextPrototype.drawImage = function (image, var_args) {
        ...
        if (sx || sy) {
          vmlStr.push(
            ...
            Math.ceil((dw + (sx * dw) / sw) * scaleX),
            'px;',
            ' height:',
            Math.ceil((dh + (sy * dh) / sh) * scaleY),
            'px;',
        ...
        vmlStr.push(
          '<div style="width:',
          Math.round((scaleX * w * dw) / sw),
          'px;',
          ' height:',
          Math.round((scaleY * h * dh) / sh),
          'px;',
          ' filter:'
        );
...
define('zrender/Painter', [
  ...
  Painter.prototype._getHeight = function () {
    ...
    return (
      (
        (root.clientHeight || parseInt(stl.height, 10)) -
...
define('zrender/loadingEffect/Base', ['require', '../tool/util', '../shape/Text', '../shape/Rectangle'], function (
  require
) {
  var util = require('../tool/util');
  ...
  Transformable.prototype = {
    ...
    updateNeedTransform: function () {
      this.needLocalTransform =
        isNotAroundZero(this.rotation[0]) ||
  ...
  Base.prototype = {
    ...
    ondrop: function (param, status) {
      ...
      if (dataIndex === -1) {
        ...
        if (data.value != null) {
          if (data.value != '-') {
            series[seriesIndex].data[dataIndex].value = accMath.accAdd(
          ...
          if (data != '-') {
            series[seriesIndex].data[dataIndex] = accMath.accAdd(
    ...
    _mapData: function (seriesArray) {
      ...
      for (var i = 0, l = seriesArray.length; i < l; i++) {
        ...
        this._sIndex2ShapeMap[seriesArray[i]] =
          this._sIndex2ShapeMap[seriesArray[i]] || this.query(serie, 'symbol') || this._symbol[i % this._symbol.length];
        if (legend) {
      ...
      for (var j = 0, k = locationMap.length; j < k; j++) {
        for (var m = 0, n = locationMap[j].length; m < n; m++) {
          ...
          if (xy.indexOf('x') != '-1') {
            ...
            var x = this.component.xAxis
              .getAxis(series[seriesIndex].xAxisIndex || 0)
          ...
          if (xy.indexOf('y') != '-1') {
            ...
            var y = this.component.yAxis
              .getAxis(series[seriesIndex].yAxisIndex || 0)
              .getCoord(tarMark['average' + valueIndex]);
            tarMark['averageLine' + valueIndex] = [
              [grid.getX(), y],
              [grid.getXend(), y],
    ...
    _buildMarkLine: function (seriesIndex) {
      ...
      for (var i = 0, l = markLine.data.length; i < l; i++) {
        ...
        if (mlData.type && (mlData.type === 'max' || mlData.type === 'min' || mlData.type === 'average')) {
          ...
          markLine.data[i][0].value =
            mlData.type !== 'average'
              ? pos[3]
              : +pos[3].toFixed(
                  markLine.precision != null
                    ? markLine.precision
                    : this.deepQuery([this.ecTheme, ecConfig], 'markLine.precision')
                );
          pos = pos[2];
          mlData = [{}, {}];
        } else {
    ...
    _markPoint: function (seriesIndex, mpOption) {
      ...
      zrUtil.merge(
        zrUtil.merge(mpOption, zrUtil.clone(this.ecTheme.markPoint || {})),
    ...
    _markLine: (function () {
      ...
      return function (seriesIndex, mlOption) {
        ...
        var legend = component.legend;
        zrUtil.merge(
          zrUtil.merge(mlOption, zrUtil.clone(this.ecTheme.markLine || {})),
        ...
        for (var i = 0; i < data.length; i++) {
          ...
          if (isCoordAvailable(mlData[0]) && isCoordAvailable(mlData[1])) {
            ...
            edges.push({
              points: [
                [this.parsePercent(mlData[0].x, zrWidth), this.parsePercent(mlData[0].y, zrHeight)],
        ...
        for (var i = 0, l = edges.length; i < l; i++) {
          ...
          ecData.pack(
            ...
            mlData[0],
            i,
            mlData[0].name + (mlData[1].name !== '' ? ' > ' + mlData[1].name : ''),
            value
          );
    ...
    getSymbolShape: function (serie, seriesIndex, data, dataIndex, name, x, y, symbol, color, emptyColor, orient) {
      ...
      var eBorderWidth =
        emphasis.borderWidth != null ? emphasis.borderWidth : emphasis.lineStyle && emphasis.lineStyle.width;
      ...
      if (this.option.animation && !this.option.renderAsImage && shapeList.length < maxLenth && !this.motionlessOnce) {
        ...
        for (key in newMap) {
          if (oldMap[key]) {
            ...
            delay =
              (this.type == ecConfig.CHART_TYPE_LINE || this.type == ecConfig.CHART_TYPE_RADAR) &&
              key.indexOf('icon') !== 0
                ? duration / 2
                : 0;
      ...
      if (this.selectedMap[serie.name]) {
        ...
        while (len--) {
          for (var i = 0, l = shapeList[len].length; i < l; i++) {
            if (
              shapeList[len][i]._mark == markType &&
...
define('echarts/util/ecEffect', [
  ...
  function point(zr, effectList, shape, zlevel) {
    ...
    if (effect.type === 'scale') {
      ...
      zr.animate(effectShape.id, 'style', effect.loop)
        .when(duration, { y: effectShape.style.y - distance })
        .when(duration * 2, { y: effectShape.style.y })
        .done(function () {
          shape.effect.show = false;
          zr.delShape(effectShape.id);
        })
  ...
  function line(zr, effectList, shape, zlevel, isLarge) {
    ...
    if (shape instanceof Polyline) {
      ...
      if (shape.style.curveness > 0) {
        ...
        effectShape.effectAnimator = zr.animation
          .animate(effectShape, { loop: effect.loop })
          .when(duration, { p: 1 })
        ...
        effectShape.effectAnimator = zr.animation
          .animate(effectShape.style, { loop: effect.loop })
          .when(duration, {
            x: x2,
            y: y2,
          })
  ...
  function largeLine(zr, effectList, shape, zlevel) {
    ...
    if (maxDuration) {
      ...
      var animator = zr
        .animate(effectShape.id, '', effect.loop)
        .when(maxDuration, { __dummy: 1 })
        .during(function () {
          zr.modShape(effectShape);
        })
        .done(function () {
...
define('echarts/component/base', [
    ...
    self._onlegendhoverlink = function (param) {
      if (self.legendHoverLink) {
        ...
        for (var i = self.shapeList.length - 1; i >= 0; i--) {
          name =
            self.type == ecConfig.CHART_TYPE_PIE || self.type == ecConfig.CHART_TYPE_FUNNEL
  ...
  Base.prototype = {
    ...
    reformOption: function (opt) {
      opt = zrUtil.merge(
        zrUtil.merge(opt || {}, zrUtil.clone(this.ecTheme[this.type] || {})),
...
define('zrender/shape/Heart', ['require', './Base', './util/PathProxy', '../tool/area', '../tool/util'], function (
  require
) {
  'use strict';
  var Base = require('./Base');
...
define('zrender/shape/Droplet', ['require', './Base', './util/PathProxy', '../tool/area', '../tool/util'], function (
  require
) {
  'use strict';
  var Base = require('./Base');
...
define('zrender/shape/util/PathProxy', ['require', '../../tool/vector'], function (require) {
  ...
  PathProxy.prototype.arc = function (cx, cy, r, startAngle, endAngle, anticlockwise) {
    this.pathCommands.push(
      new PathSegment('A', [cx, cy, r, r, startAngle, endAngle - startAngle, 0, anticlockwise ? 0 : 1])
    );
    if (this._ctx) {
...
define('echarts/component/dataView', ['require', './base', '../config', 'zrender/tool/util', '../component'], function (
  require
) {
  var Base = require('./base');
  var ecConfig = require('../config');
  ...
  DataView.prototype = {
    ...
    show: function (newOption) {
      ...
      this.option = newOption;
      this._tDom.innerHTML =
        '<p style="padding:8px 0;margin:0 0 10px 0;' +
      ...
      this._sizeCssText =
        'width:' + this._zrWidth + 'px;' + 'height:' + this._zrHeight + 'px;' + 'background-color:#fff;';
      this._tDom.style.cssText = this._gCssText + this._sizeCssText;
    },
    ...
    _save: function () {
      ...
      setTimeout(
        function () {
          self.messageCenter &&
            self.messageCenter.dispatch(ecConfig.EVENT.DATA_VIEW_CHANGED, null, { option: self.option }, self.myChart);
        },
        ...
        if (this._axisList[i]) {
          ...
          this._axisList[i] =
            axisOption[i].type === 'category'
              ? new CategoryAxis(this.ecTheme, this.messageCenter, this.zr, axisOption[i], this.myChart, this.axisBase)
              : new ValueAxis(
                  this.ecTheme,
                  this.messageCenter,
                  this.zr,
  ...
  DataZoom.prototype = {
    ...
    _getZoom: function () {
      ...
      for (var i = 0, l = series.length; i < l; i++) {
        ...
        if (
          this.zoomOption.xAxisIndex == null &&
      ...
      var start =
        this._zoom.start != null ? this._zoom.start : this.zoomOption.start != null ? this.zoomOption.start : 0;
      ...
      var size = Math.round(
        ((end - start) / 100) * (this.zoomOption.orient == 'horizontal' ? this._location.width : this._location.height)
      );
      return {
        start: start,
        end: end,
    ...
    _buildHandle: function () {
      var detail = this.zoomOption.showDetail
        ? this._getDetail()
        : {
            start: '',
            end: '',
    ...
    _syncHandleShape: function () {
      if (this.zoomOption.orient == 'horizontal') {
        ...
        this._zoom.end =
          ((this._location.y + this._location.height - this._endShape.style.y - this._handleSize) /
    ...
    _syncFillerShape: function () {
      ...
      if (this.zoomOption.orient == 'horizontal') {
        ...
        this._zoom.end =
          ((this._location.y + this._location.height - Math.min(a, b) - this._handleSize) / this._location.height) *
          100;
      }
    ...
    _syncShape: function () {
      ...
      if (this.zoomOption.orient == 'horizontal') {
        ...
        this._startShape.style.y =
          this._location.y + this._location.height - (this._zoom.start / 100) * this._location.height;
        this._endShape.style.y =
          this._location.y + this._location.height - (this._zoom.end / 100) * this._location.height - this._handleSize;
    ...
    _syncData: function (dispatchNow) {
      ...
      for (var key in this._originalData) {
        ...
        for (var idx in target) {
          ...
          if (
            !(this.getDataFromOption(data[0]) instanceof Array) ||
    ...
    _setScale: function () {
      var needScale =
        this._zoom.start !== 0 || this._zoom.end !== 100 || this._zoom.start2 !== 0 || this._zoom.end2 !== 100;
      var axis = {
  ...
  CategoryAxis.prototype = {
              ...
              for (var i = Math.floor((dataLength - 1) / interval) * interval; i >= 0; i -= interval) {
                if (this.option.axisLabel.rotate !== 0) {
                  ...
                  labelSize = zrArea.getTextWidth(
                    ...
                    this.getFont(zrUtil.merge(data[i].textStyle, this.option.axisLabel.textStyle))
                  );
                } else {
    ...
    _buildAxisTick: function () {
      ...
      var intervalFunction =
        typeof tickOption.interval == 'function'
          ? tickOption.interval
      ...
      if (this.isHorizontal()) {
        var yPosition =
          this.option.position == 'bottom'
            ? tickOption.inside
              ? this.grid.getYend() - length - 1
              : this.grid.getYend() + 1
            : tickOption.inside
            ? this.grid.getY() + 1
            : this.grid.getY() - length - 1;
        var x;
        ...
        var xPosition =
          this.option.position == 'left'
            ? tickOption.inside
              ? this.grid.getX() + 1
              : this.grid.getX() - length - 1
            : tickOption.inside
            ? this.grid.getXend() - length - 1
            : this.grid.getXend() + 1;
        var y;
        for (var i = startIndex; i < dataLength; i += interval) {
          if (intervalFunction && !intervalFunction(i, data[i])) {
        ...
        for (var i = 0; i < dataLength; i += this._interval) {
          ...
          axShape = {
            ...
            style: {
              ...
              textBaseline:
                dataTextStyle.baseline || (i === 0 && this.option.name !== '')
                  ? 'bottom'
                  : i == dataLength - 1 && this.option.name !== ''
                  ? 'top'
                  : 'middle',
            },
          };
    ...
    _buildSplitLine: function () {
      ...
      var intervalFunction =
        typeof this.option.axisLabel.interval == 'function' ? this.option.axisLabel.interval : false;
  ...
  ValueAxis.prototype = {
    ...
    _calculateValue: function () {
      if (isNaN(this.option.min - 0) || isNaN(this.option.max - 0)) {
        ...
        for (var i = 0, l = this.series.length; i < l; i++) {
          if (
            this.series[i].type != ecConfig.CHART_TYPE_LINE &&
    ...
    _reformLabelData: function (innerFormatter) {
      ...
      if (formatter) {
        ...
        for (var i = 0, l = this._valueList.length; i < l; i++) {
          this._valueLabel.push(
            innerFormatter ? innerFormatter(this._valueList[i]) : this.numAddCommas(this._valueList[i])
          );
        }
      }
    },
...
define('echarts/util/smartLogSteps', ['require', './number'], function (require) {
  ...
  function makeTicksList() {
    ...
    if (logLabelMode === 'exponent') {
      ...
      !(spanExpon <= MIN_BASE_10_SPLIT_NUMBER && splitNumber > MIN_BASE_10_SPLIT_NUMBER)
        ? baseAnalysis()
        : detailAnalysis();
    }
